...
import static com.selenium.ui.browser.DriverType.CHROME;
...
public class DriverFactoryManager {
    ...
    private final boolean chromeDriverService = Boolean.valueOf(JsonConnector.getConfig("chromeDriverService")!=null?JsonConnector.getConfig("chromeDriverService"):"false");
    ...
    private void instantiateWebDriver(DriverType driverType) throws MalformedURLException {
        ...
        if (useGrid) {
            URL seleniumGridURL = new URL(JsonConnector.getConfig("gridURL"));
        ...
        } else if(useRemoteWebDriver && chromeDriverService){	 	
        	driver = new RemoteWebDriver(ChromeService.chService.getUrl(), desiredCapabilities);
        }else {
